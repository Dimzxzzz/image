...
class SecurityController extends Controller
{
    ...
    public function index()
    {
        ...
        $accessLogs = [];
        $accessLogFile = storage_path('logs/access.log');
        if (file_exists($accessLogFile)) {
            $logs = @shell_exec("tail -50 $accessLogFile 2>/dev/null | head -20");
            if ($logs) {
                $accessLogs = array_filter(explode("\n", trim($logs)));
            }
        }
    ...
    public function bannedIps(Request $request)
    {
        ...
        if ($search) {
            $ips->where('ip_address', 'like', "%{$search}%")
    ...
    public function rateLimits()
    {
        $limits = [
            ...
            [
                ...
                'enabled' => Cache::get('rate_limit:enabled:websocket', true),
                'config' => Cache::get('rate_limit:config:websocket', ['max' => 10, 'window' => 60])
            ]
        ];
    ...
    public function getStats()
    {
        ...
        $stats = [
            ...
            'rate_limits' => [
                ...
                'login' => Cache::get('rate_limit:enabled:login', true),
                'files' => Cache::get('rate_limit:enabled:files', true),
                'websocket' => Cache::get('rate_limit:enabled:websocket', true),
            ]
        ];
        ...
        <div class="info-box bg-green">
            ...
            <div class="info-box-content">
                ...
                <span class="info-box-number">
                    {{ array_sum(array_map(function($v) { return $v ? 1 : 0; }, $rateLimits)) }}/4
                </span>
                <div class="progress">
                    @php $active = array_sum(array_map(function($v) { return $v ? 1 : 0; }, $rateLimits)); @endphp
                    <div class="progress-bar" style="width: {{ ($active/4)*100 }}%"></div>
                </div>
                <span class="progress-description">
                    Protection enabled
                </span>
...
<div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="small-box bg-{{ $enabled ? 'green' : 'gray' }}">
                            <div class="inner">
                                <h4 style="margin: 0">{{ ucfirst($key) }}</h4>
    ...
    <div class="col-md-6">
        ...
        <div class="box box-default">
            <div class="box-header with-border">
                <h3 class="box-title">Recent Access Logs</h3>
                <div class="box-tools">
                    <button class="btn btn-xs btn-default" data-toggle="modal" data-target="#clearLogsModal">
                        Clear Logs
                    </button>
                </div>
            </div>
            <div class="box-body" style="max-height: 300px; overflow-y: auto;">
                ...
                <ul class="list-unstyled">
                    ...
                    <li style="padding: 5px 0; border-bottom: 1px solid #f4f4f4; font-family: monospace; font-size: 12px;">
                        {{ $log }}
                    </li>
                    @endforeach
                </ul>
                @else
                <p class="text-muted">No access logs available. Ensure logging is enabled.</p>
                @endif
            </div>
        </div>
            ...
            <form action="{{ route('admin.security.ban-ip') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <small class="text-muted">Supports single IP or CIDR notation</small>
                    </div>
                    <div class="form-group">
    ...
    $.ajax({
        ...
        success: function(data) {
            alert('Security Statistics:\n\n' +
                  ...
                  '- WebSocket: ' + (data.rate_limits.websocket ? 'Enabled' : 'Disabled'));
        }
    });
}
</script>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($ip->expires_at && $ip->expires_at->isPast())
    ...
    <div class="col-md-12">
        <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
                ...
                <li><a href="#logs" data-toggle="tab">Monitoring</a></li>
            </ul>
            <div class="tab-content">
                    ...
                    <div class="box box-info">
                        ...
                        <div class="box-body">
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <button class="btn btn-block btn-warning" onclick="resetToDefaults()">
                                        <i class="fa fa-undo"></i> Reset Defaults
                                    </button>
                                </div>
                                <div class="col-md-3">
                                    <button class="btn btn-block btn-info" onclick="testRateLimits()">
                        ...
                        <div class="box-body">
                            ...
                            <div class="callout callout-warning">
                                ...
                                <ul>
                                    ...
                                    <li><strong>WebSocket:</strong> 5-10 connections per minute per IP</li>
                                </ul>
                            </div>
                ...
                <div class="tab-pane" id="logs">
                    <div class="box box-default">
                        <div class="box-header with-border">
                            <h3 class="box-title">Rate Limit Monitoring</h3>
                            <div class="box-tools">
                        ...
                        <div class="box-body">
                            <div class="alert alert-info">
                                <i class="fa fa-info-circle"></i> Rate limit logs show when clients exceed limits. Monitor these to detect abuse patterns.
                            </div>
                            
                            <div id="rateLimitLogs">
...
function resetToDefaults() {
    if (confirm('Reset all rate limits to default values?')) {
        $.ajax({
            url: '{{ route('admin.security.reset-defaults') }}',
            method: 'POST',
...
function testRateLimits() {
    $.ajax({
        url: '{{ route('admin.security.test-limits') }}',
        method: 'GET',
...
function refreshLogs() {
    $('#rateLimitLogs').html('<p class="text-muted">Loading logs...</p>');
    $.ajax({
        url: '{{ route('admin.security.get-logs') }}',
        method: 'GET',
        success: function(data) {
            if (data.logs && data.logs.length > 0) {
                var html = '<div class="table-responsive"><table class="table table-hover"><thead><tr><th>Time</th><th>IP Address</th><th>Endpoint</th><th>Limit Type</th><th>Action</th></tr></thead><tbody>';
                data.logs.forEach(function(log) {
                    html += '<tr><td>' + log.time + '</td><td><code>' + log.ip + '</code></td><td>' + log.endpoint + '</td><td>' + log.type + '</td><td><span class="label label-danger">BLOCKED</span></td></tr>';
                });
                html += '</tbody></table></div>';
                $('#rateLimitLogs').html(html);
            } else {
                $('#rateLimitLogs').html('<div class="alert alert-success"><i class="fa fa-check"></i> No rate limit violations detected.</div>');
            }
        }
    });
}
...
if [ -f "$SIDEBAR_FILE" ]; then
    ...
    if ! grep -q "Security Settings" "$SIDEBAR_FILE"; then
        sed -i '/<a href="{{ route(.admin.settings.) }}">.*<\/a>/a \
                @if(auth()->user()->id === 1)\
    ...
    ALTERNATIVE_SIDEBAR="/var/www/pterodactyl/resources/views/admin/security/_sidebar.blade.php"
    cat > "$ALTERNATIVE_SIDEBAR" << 'EOF'
@if(auth()->user()->id === 1)
<li class="{{ $active === 'security' ? 'active' : '' }}">
    <a href="{{ route('admin.security') }}">
        <i class="fa fa-shield"></i> <span>Security Settings</span>
...
EOF
    ...
    echo "Created sidebar include at: $ALTERNATIVE_SIDEBAR"
    echo "You need to manually include this file in your admin layout."
fi
...
echo "1. Security Dashboard - Monitor semua aktivitas"
echo "2. Banned IP Management - Ban/Unban IP address"
echo "3. Rate Limit Control - Kontrol limit request"
echo "4. Suspicious Activity Monitoring"
echo "5. Access Logs Viewer"
echo "6. Mass Actions - Unban multiple IPs sekaligus"
echo "7. Export banned IPs list"
echo ""
echo "CARA AKSES:"
echo "1. Login sebagai OWNER (user ID 1)"
echo "2. Buka Admin Panel"
echo "3. Lihat menu 'Security Settings' di sidebar"
echo ""
echo "TROUBLESHOOTING:"
echo "- php artisan route:list | grep security"
echo "- php artisan cache:clear"
echo "- tail -f storage/logs/laravel.log"
echo "- Cek permissions: ls -la /var/www/pterodactyl"
echo ""
